"""
atlas_hand_core/sources/atlas_glove.py
AGA 글러브 OSC 입력 소스
"""
...
class AtlasGloveSource(HandInputSource):
    """AGA 글러브 OSC 입력 소스.
    ...
    def __init__(
        self,
        listen_ip: str = OSC_LISTEN_IP,
        server_port: int = OSC_SERVER_PORT,
        target_ip: str = OSC_TARGET_IP,
    ...
    def start(self) -> None:
        ...
        self._running = True
        threading.Thread(
            target=self._server.serve_forever, daemon=True, name='AtlasOSCServer'
        ).start()

        self._udp_client = SimpleUDPClient(self._target_ip, self._client_port)
        threading.Thread(
            target=self._heartbeat_loop, daemon=True, name='AtlasHeartbeat'
        ).start()

        _log.info(f'[Atlas] OSC 수신 대기: {self._listen_ip}:{self._server_port}')
        _log.info(f'[Atlas] 하트비트 -> {self._target_ip}:{self._client_port} ({HEARTBEAT_INTERVAL_SEC}s)')
    ...
    def send_haptic(self, side: str, values: list) -> bool:
        ...
        try:
            ...
            if self._verbose:
                _log.debug(f'[Atlas] -> {side} hapt  {[int(v) for v in values[:AGA_FINGER_COUNT]]}')
            return True
        except Exception as e:
            _log.warning(f'[Atlas] {side} 햅틱 전송 실패: {e}')
            return False
    ...
    def _h_left_quat(self, address: str, *args) -> None:
        ...
        if raw is None:
            _log.warning(f'[Atlas] {address} 파싱 실패 (args={len(args)})')
            return
        ...
        if self._verbose:
            _log.debug(f'[Atlas] <- left quat  raw[:4]={raw[:4].round(3)}')
    ...
    def _h_right_quat(self, address: str, *args) -> None:
        ...
        if self._verbose:
            _log.debug(f'[Atlas] <- right quat  raw[:4]={raw[:4].round(3)}')
    ...
    def _h_device_status(self, address: str, *args) -> None:
        ...
        if self._verbose:
            _log.debug(f'[Atlas] device status  left={self._left_connected}  right={self._right_connected}')

    def _h_echo(self, address: str, *args) -> None:
        if self._verbose:
            _log.debug(f'[Atlas] <- {address}  args={args}')

    def _h_alarm(self, address: str, *args) -> None:
        _log.warning(f'[Atlas] ALARM <- {address}  args={args}')

    def _h_default(self, address: str, *args) -> None:
        if self._verbose:
            _log.debug(f'[Atlas] 미처리 주소: {address}  args={args}')
    ...
    def _heartbeat_loop(self) -> None:
        while self._running:
            try:
                ...
                if self._verbose:
                    _log.debug('[Atlas] -> heartbeat sent')
            except Exception as e:
                _log.warning(f'[Atlas] heartbeat 전송 실패: {e}')
